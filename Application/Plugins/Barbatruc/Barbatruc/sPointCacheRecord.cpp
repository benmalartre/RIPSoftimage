...
XSIPLUGINCALLBACK CStatus bbppPointCacheRecord_Init( CRef& in_ctxt )
{
...
XSIPLUGINCALLBACK CStatus bbppPointCacheRecord_Execute( CRef& in_ctxt )
{
	Application app;
	Context ctxt( in_ctxt );
	...
	ProgressBar.PutCaption( L"bPointCache ---> Write cache for "+ oObj.GetFullName());
	...
	if(format == 0)
	{
		char signature[SIZE_SIGNATURE] = SIGNATURE_POINTCACHE;
		out_file.write((char*) &signature, SIZE_SIGNATURE);
...
bool FileExists(CString inFilename) 
{
  ...
  if(intStat == 0) 
  {
	...
	CString title = L"bWritePointCache";
	long out;
