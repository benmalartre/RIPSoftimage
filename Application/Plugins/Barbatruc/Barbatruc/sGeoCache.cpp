...
XSIPLUGINCALLBACK CStatus bbppGeoCache_Define( CRef& in_ctxt )
{
...
CStatus sGeoCache::Update(OperatorContext& in_ctxt)
{
...
XSIPLUGINCALLBACK CStatus bbppGeoCache_Init( CRef& in_ctx )
{
	Context ctx(in_ctx) ;	
	sGeoCache* pThis = new sGeoCache();	
	ctx.PutUserData( (CValue::siPtrType)pThis );
	CustomOperator oOp = ctx.GetSource();
	return CStatus::OK;
}

XSIPLUGINCALLBACK CStatus bbppGeoCache_Term( CRef& in_ctx)
{
	Context ctx(in_ctx) ;
	CValue::siPtrType pUserData = ctx.GetUserData();
	sGeoCache* pThis = (sGeoCache*)pUserData;
...
XSIPLUGINCALLBACK CStatus bbppGeoCache_Update(CRef&	in_ctx)
{
