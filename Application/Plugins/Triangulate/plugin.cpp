...
inline int XYZCompare(const void *v1, const void *v2)
{
	XYZ *p1, *p2;
	p1 = (XYZ *)v1;
	p2 = (XYZ *)v2;
...
extern int CircumCircle(double xp,double yp,
...
void construct_plane(plane& P,CVector3f& p1,CVector3f& p2,CVector3f& p3){
...
bool is_in_half_space(plane& P,CVector3f& point){
...
bool is_in_frustum(plane& left,plane& right, plane& up, plane& down, plane& nearp,plane& farp,CVector3f& point){
	if(	is_in_half_space(left,point) &&
		is_in_half_space(right,point) &&
		is_in_half_space(up,point) &&
		is_in_half_space(down,point) && 
		is_in_half_space(nearp,point) && 
		is_in_half_space(farp,point) 
	)
			return true;
...
SICALLBACK Triangulate_BeginEvaluate( ICENodeContext& in_ctxt )
{
	...
	Application().LogMessage(L"Begin Evaluate Called...");
	...
	construct_plane(left_plane,NearLD,FarLD ,FarLU);
	construct_plane(right_plane,FarRD,NearRD,FarRU);
	construct_plane(up_plane,NearRU,NearLU,FarRU);
	construct_plane(down_plane,NearLD,NearRD,FarRD);
	construct_plane(near_plane,NearLD,NearLU,NearRD);
	construct_plane(far_plane,FarLU,FarRU,FarRD);
	//----------------------------------------------------------------------------------------------------------
