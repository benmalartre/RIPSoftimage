...
ITriangulate<T, I>::~ITriangulate()
{
}

template <typename T, typename I>
struct CTriangulate : ITriangulate<T, I>
{
  CTriangulate() : vert_map(0),
    vert_alloc(0),
    face_alloc(0),
    max_verts(0),
    max_faces(0),
  ...
  struct Iter : ITriangulate<T, I>::Iterator
  {
  };

  struct Vert : ITriangulate<T, I>::Vertex
  {
  ...
  struct Face : ITriangulate<T, I>::Simplex
  {
  ...
  virtual I FloodFill(bool invert, const typename ITriangulate<T, I>::Simplex** exterior)
  {
    if (!first_dela_face)
      return 0;
  ...
  virtual I Polygonize(const typename ITriangulate<T, I>::Simplex* poly[])
  {
    const I marker = (I)(-1);
    ...
    if (!poly)
    {
      ...
      poly = (const typename ITriangulate<T, I>::Simplex**)buf;
      if (!poly)
        return -1;
    }
  ...
  virtual const typename ITriangulate<T, I>::Simplex* GetFirstDelaunaySimplex() const
  {
    return first_dela_face;
  }

  virtual const typename ITriangulate<T, I>::Simplex* GetFirstHullSimplex() const
  {
    return first_hull_face;
  }

  virtual const typename ITriangulate<T, I>::Vertex* GetFirstBoundaryVertex() const
  {
    return first_boundary_vert;
  }

  virtual const typename ITriangulate<T, I>::Vertex* GetFirstInternalVertex() const
  {
    return first_internal_vert;
  }

  virtual const typename ITriangulate<T, I>::Vertex* GetVertexByIndex(I i) const
  {
    if (i < 0 || i >= inp_verts)
      return 0;
...
template <typename T, typename I>
ITriangulate<T, I>* ITriangulate<T, I>::Create()
{
  ITriangulate<T, I>* ret = 0;
  try
  {
    ret = new CTriangulate<T, I>;
  }
  catch (...)
  {
    ret = 0;
...
template <typename T, typename I>
const T CTriangulate<T, I>::Face::iccerrboundA = ((T(10) + T(96) * std::exp2(-(T)std::numeric_limits<T>::digits)) * std::exp2(-(T)std::numeric_limits<T>::digits));

template <typename T, typename I>
const T CTriangulate<T, I>::Vert::resulterrbound = (T(3) + T(8) * std::exp2(-(T)std::numeric_limits<T>::digits)) * std::exp2(-(T)std::numeric_limits<T>::digits);

// this should cover all malcontents
template ITriangulate<float, int8_t>* ITriangulate<float, int8_t>::Create();
template ITriangulate<double, int8_t>* ITriangulate<double, int8_t>::Create();
template ITriangulate<long double, int8_t>* ITriangulate<long double, int8_t>::Create();

template ITriangulate<float, int16_t>* ITriangulate<float, int16_t>::Create();
template ITriangulate<double, int16_t>* ITriangulate<double, int16_t>::Create();
template ITriangulate<long double, int16_t>* ITriangulate<long double, int16_t>::Create();

template ITriangulate<float, int32_t>* ITriangulate<float, int32_t>::Create();
template ITriangulate<double, int32_t>* ITriangulate<double, int32_t>::Create();
template ITriangulate<long double, int32_t>* ITriangulate<long double, int32_t>::Create();

template ITriangulate<float, int64_t>* ITriangulate<float, int64_t>::Create();
template ITriangulate<double, int64_t>* ITriangulate<double, int64_t>::Create();
template ITriangulate<long double, int64_t>* ITriangulate<long double, int64_t>::Create();
