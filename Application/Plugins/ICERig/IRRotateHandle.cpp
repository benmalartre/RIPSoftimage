...
namespace ICERIG {
  ...
  void IRRotateHandle::MouseDown(ToolContext& in_ctxt)
  {
    ...
    double dScale = GetViewScaleFactor(in_ctxt, l_bbox.GetMidPoint(BBox::kBack, true), kRotateStickLength);
    if (dScale == 0.0)
      return;

    // Initialize arcball rotation controller
    m_pivot = l_bbox.GetCenter(true);
    m_constraintAxes[0] = l_bbox.GetNormal(BBox::kRight, true);
    m_constraintAxes[1] = l_bbox.GetNormal(BBox::kTop, true);
