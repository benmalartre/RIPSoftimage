  ...
  CLongArray &in_PolyPointCount, CLongArray &in_PolyPointIndice,  std::vector<DMPoint_t> *io_ArrayPoint )
{
  ...
  for ( ULONG iPoly=0 ; iPoly<in_PolyCount ; iPoly++ ) {
    ...
    for ( ULONG iPnt=0 ; iPnt<(ULONG)currentPointNumber ; iPnt++, nPolyOffset++ ) {
      ...
      io_ArrayPoint->at(currentPointIndex).position.Set (
        (float)in_GeoPointPosition[currentPointIndex*3],
...
void ComputePointReferenceFrame ( std::vector<DMPoint_t> *io_ArrayPoint, ULONG &in_PointIndex )
{
  ...
  io_ArrayPoint->at(in_PointIndex).normal.ScaleInPlace ( 
    1.0f / (float)io_ArrayPoint->at(in_PointIndex).arrayNeigborsEdgePointIndex.size() );
  io_ArrayPoint->at(in_PointIndex).normal.NormalizeInPlace();

  //Compute Tangent and BiNormal.
  io_ArrayPoint->at(in_PointIndex).tangent.Sub (
    io_ArrayPoint->at(in_PointIndex).smoothPosition, 
    io_ArrayPoint->at(io_ArrayPoint->at(in_PointIndex).arrayNeigborsEdgePointIndex.at(
      io_ArrayPoint->at(in_PointIndex).arrayNeigborsEdgePointIndex.size()-1).p2).smoothPosition 
  );

  io_ArrayPoint->at(in_PointIndex).biNormal.Cross ( 
    io_ArrayPoint->at(in_PointIndex).tangent, 
    io_ArrayPoint->at(in_PointIndex).normal 
  );
  io_ArrayPoint->at(in_PointIndex).tangent.Cross ( 
    io_ArrayPoint->at(in_PointIndex).normal, 
    io_ArrayPoint->at(in_PointIndex).biNormal 
  );
  ...
  io_ArrayPoint->at(in_PointIndex).referenceFrame.Set(
    io_ArrayPoint->at(in_PointIndex).tangent.GetX(), 
    io_ArrayPoint->at(in_PointIndex).tangent.GetY(), 
    io_ArrayPoint->at(in_PointIndex).tangent.GetZ(),
    io_ArrayPoint->at(in_PointIndex).normal.GetX(), 
    io_ArrayPoint->at(in_PointIndex).normal.GetY(), 
    io_ArrayPoint->at(in_PointIndex).normal.GetZ(),
    io_ArrayPoint->at(in_PointIndex).biNormal.GetX(), 
    io_ArrayPoint->at(in_PointIndex).biNormal.GetY(), 
    io_ArrayPoint->at(in_PointIndex).biNormal.GetZ() 
  );
};
...
void ComputePointMushInitData ( std::vector<DMPoint_t> *io_ArrayPoint )
{
  for ( ULONG iPnt=0 ; iPnt<io_ArrayPoint->size() ; iPnt++ ) {
    ...
    io_ArrayPoint->at(iPnt).offsetVector.Sub (
      io_ArrayPoint->at(iPnt).position, 
      io_ArrayPoint->at(iPnt).smoothPosition
    );

    io_ArrayPoint->at(iPnt).transform.Set(
      io_ArrayPoint->at(iPnt).tangent.GetX(), 
      io_ArrayPoint->at(iPnt).tangent.GetY(), 
      io_ArrayPoint->at(iPnt).tangent.GetZ(), 
      0.0f,
      io_ArrayPoint->at(iPnt).normal.GetX(), 
      io_ArrayPoint->at(iPnt).normal.GetY(), 
      io_ArrayPoint->at(iPnt).normal.GetZ(), 
      0.0f,
      io_ArrayPoint->at(iPnt).biNormal.GetX(), 
      io_ArrayPoint->at(iPnt).biNormal.GetY(), 
      io_ArrayPoint->at(iPnt).biNormal.GetZ(), 
      0.0f,
      io_ArrayPoint->at(iPnt).offsetVector.GetX(), 
      io_ArrayPoint->at(iPnt).offsetVector.GetY(), 
      io_ArrayPoint->at(iPnt).offsetVector.GetZ(), 
      1.0f 
    );
  }
};
...
void ComputePointMushData ( std::vector<DMPoint_t> *io_ArrayPoint, CDataArray2DMatrix4f &in_DmInitData )
{
  ...
  for ( ULONG iPnt=0 ; iPnt<io_ArrayPoint->size() ; iPnt++ ) {
    ...
    offsetRotation.Set (
      arrayDmInitData[iPnt].GetValue(0,0),
      arrayDmInitData[iPnt].GetValue(0,1),
      arrayDmInitData[iPnt].GetValue(0,2),
      arrayDmInitData[iPnt].GetValue(1,0), 
      arrayDmInitData[iPnt].GetValue(1,1),
      arrayDmInitData[iPnt].GetValue(1,2),
      arrayDmInitData[iPnt].GetValue(2,0), 
      arrayDmInitData[iPnt].GetValue(2,1),
      arrayDmInitData[iPnt].GetValue(2,2) 
    );

    offsetVector.Set(
      arrayDmInitData[iPnt].GetValue(3,0), 
      arrayDmInitData[iPnt].GetValue(3,1),
      arrayDmInitData[iPnt].GetValue(3,2)
    );
