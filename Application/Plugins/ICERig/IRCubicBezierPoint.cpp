...
CStatus RegisterIRCubicBezierPoint( PluginRegistrar& in_reg )
{
  ...
  st = nodeDef.AddInputPort(
    ID_IN_U, ID_G_100, siICENodeDataFloat, siICENodeStructureSingle, siICENodeContextAny, 
    L"U", L"U", 0, 0, 1, ID_UNDEF, ID_UNDEF, ID_CTXT_CNS
  );
  st.AssertSucceeded();

  // Add output ports.
  st = nodeDef.AddOutputPort(
    ID_OUT_Position,siICENodeDataVector3, siICENodeStructureSingle, siICENodeContextAny,
    L"Position", L"Position", ID_UNDEF, ID_UNDEF, ID_CTXT_CNS
  );
  st.AssertSucceeded( ) ;

  st = nodeDef.AddOutputPort(
    ID_OUT_Tangent, siICENodeDataVector3, siICENodeStructureSingle, siICENodeContextAny, 
