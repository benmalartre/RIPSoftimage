...
namespace ICERIG {
	...
	CVector3 IRJoint::GetTangent()
	{
		// first point
		if (m_prev == NULL)
			m_tangent.Sub(m_next->GetOffsetPosition(), GetOffsetPosition());

		// last point
		else if (m_next == NULL)
		{
			m_tangent.Sub(GetOffsetPosition(), m_prev->GetOffsetPosition());
		}
		else
		{
			CVector3 a, b;
			a.Sub(GetOffsetPosition(), m_prev->GetOffsetPosition());
			b.Sub(m_next->GetOffsetPosition(), GetOffsetPosition());
