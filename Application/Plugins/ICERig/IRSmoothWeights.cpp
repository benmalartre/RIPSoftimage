...
CStatus RegisterIRSmoothWeights( PluginRegistrar& in_reg )
{
	...
	st = nodeDef.AddInputPort(ID_IN_PointWeight,ID_G_100,siICENodeDataFloat,siICENodeStructureArray,siICENodeContextComponent0D,L"PointWeights",L"PointWeights",0,CValue(),CValue(),ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
	st.AssertSucceeded( );

	st = nodeDef.AddInputPort(ID_IN_NeighborIDs,ID_G_100,siICENodeDataLong,siICENodeStructureArray,siICENodeContextComponent0D,L"NeighborIDs",L"NeighborIDs",0,CValue(),CValue(),ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
	st.AssertSucceeded( ) ;

	st = nodeDef.AddInputPort(ID_IN_Iterations,ID_G_100,siICENodeDataLong,siICENodeStructureSingle,siICENodeContextSingleton,L"Iterations",L"Iterations",0,CValue(),CValue(),ID_UNDEF,ID_UNDEF,ID_UNDEF);
	st.AssertSucceeded( ) ;

	st = nodeDef.AddInputPort(ID_IN_SmoothMap,ID_G_100,siICENodeDataFloat,siICENodeStructureSingle,siICENodeContextComponent0D,L"SmoothMap",L"SmoothMap",0,CValue(),CValue(),ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
	st.AssertSucceeded( ) ;

	// Add output ports.
	st = nodeDef.AddOutputPort(ID_OUT_OutWeights,siICENodeDataFloat,siICENodeStructureArray,siICENodeContextComponent0D,L"OutWeights",L"OutWeights",ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
	st.AssertSucceeded( ) ;
...
SICALLBACK IRSmoothWeights_Evaluate( ICENodeContext& in_ctxt )
{
	...
	// Get User Data
	Smooth* smoot = (Smooth*)(CValue::siPtrType)in_ctxt.GetUserData( );
	...
	//Application().LogMessage(L" Nb Points : "+(CString)SmoothMapData.GetCount());
	
	if(!smoot->_init)
		smoot->Init(SmoothMapData.GetCount(),IterationsData[0]);

	smoot->iterations = IterationsData[0];
	...
	{
		SmoothPoint* p = smoot->_points[it.GetAbsoluteIndex()];
	...
	for(CIndexSet::Iterator it = indexSet.Begin(); it.HasNext(); it.Next())
	{
		p = smoot->_points[it.GetAbsoluteIndex()];
...
SICALLBACK IRSmoothWeights_Init( CRef& in_ctxt )
{
	...
	Smooth* smoot = new Smooth();
	smoot->_init = false;
	ctxt.PutUserData( (CValue::siPtrType)smoot );
	return CStatus::OK;
}
