...
CStatus RegisterIRFabrik( PluginRegistrar& in_reg )
{
		...
		st = nodeDef.AddInputPort(ID_IN_Ts,ID_G_100,siICENodeDataMatrix44,siICENodeStructureArray,siICENodeContextSingleton,L"Ts",L"Ts",CMatrix4f(),CMatrix4f(),CMatrix4f(),ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
        st.AssertSucceeded( ) ;

        st = nodeDef.AddInputPort(ID_IN_Pos,ID_G_100,siICENodeDataVector3,siICENodeStructureSingle,siICENodeContextSingleton,L"Position",L"Position",CVector3f(),CVector3f(),CVector3f(),ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
        st.AssertSucceeded( ) ;

		st = nodeDef.AddInputPort(ID_IN_Iteration,ID_G_100,siICENodeDataLong,siICENodeStructureSingle,siICENodeContextSingleton,L"Iteration",L"Iteration",CValue(),CValue(),CValue(),ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
        st.AssertSucceeded( ) ;

        // Add output ports.
        st = nodeDef.AddOutputPort(ID_OUT_T,siICENodeDataMatrix44,siICENodeStructureArray,siICENodeContextSingleton,L"OutT",L"OutT",ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
        st.AssertSucceeded( ) ;
...
SICALLBACK IRFabrik_Evaluate( ICENodeContext& in_ctxt )
{
		...
		for(int i=nbt;i-->0;)
		{
			ascT[i] = accessor[i];
			if(i>0){
				s.Set(accessor[i-1].GetValue(3,0),accessor[i-1].GetValue(3,1),accessor[i-1].GetValue(3,2));
				e.Set(accessor[i].GetValue(3,0),accessor[i].GetValue(3,1),accessor[i].GetValue(3,2));
				d.Sub(e,s);
