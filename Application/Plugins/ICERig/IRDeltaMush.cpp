...
CStatus	RegisterIRDeltaMush ( PluginRegistrar& in_reg )
{
  ...
  st = nodeDef.AddInputPort(
    DM_ID_IN_Geometry, DM_ID_G_100, siICENodeDataGeometry, siICENodeStructureSingle, siICENodeContextComponent0D,
    L"Geometry", L"Geometry", CValue(), CValue(), CValue(), DM_ID_UNDEF, DM_ID_UNDEF, DM_ID_CTXT_CNS
  );
  st.AssertSucceeded( ) ;

  st = nodeDef.AddInputPort(
    DM_ID_IN_PointIndex, DM_ID_G_100, siICENodeDataLong, siICENodeStructureSingle, siICENodeContextComponent0D,
    L"PointIndex", L"PointIndex", 10, 0, 100, DM_ID_UNDEF, DM_ID_UNDEF, DM_ID_CTXT_CNS
  );
  st.AssertSucceeded( ) ;

  st = nodeDef.AddInputPort(
    DM_ID_IN_Mush_Iterations, DM_ID_G_100, siICENodeDataLong, siICENodeStructureSingle, siICENodeContextComponent0D,
    L"Mush_Iterations", L"Mush_Iterations", 10, 0, 100, DM_ID_UNDEF, DM_ID_UNDEF, DM_ID_CTXT_CNS
  );
  st.AssertSucceeded( ) ;

  st = nodeDef.AddInputPort(
    DM_ID_IN_Smooth_Iterations, DM_ID_G_100, siICENodeDataLong, siICENodeStructureSingle, siICENodeContextComponent0D,
    L"Smooth_Iterations", L"Smooth_Iterations", 4, 0, 100, DM_ID_UNDEF, DM_ID_UNDEF, DM_ID_CTXT_CNS
  );
  st.AssertSucceeded( ) ;

  st = nodeDef.AddInputPort(
    DM_ID_IN_DM_Init_Data, DM_ID_G_100, siICENodeDataMatrix44, siICENodeStructureArray, siICENodeContextSingleton,
    L"DM_Offset_Position", L"DM_Offset_Position", 8
  );
  st.AssertSucceeded( ) ;
	
  st = nodeDef.AddInputPort(
    DM_ID_IN_Blend, DM_ID_G_100, siICENodeDataFloat, siICENodeStructureArray, siICENodeContextSingleton, 
    L"Blend", L"Blend", 16
  );
  st.AssertSucceeded( ) ;
  ...
  st = nodeDef.AddOutputPort(
    DM_ID_OUT_OutPort, siICENodeDataVector3, siICENodeStructureSingle, siICENodeContextComponent0D,
    L"OutPort", L"OutPort"
  );
  st.AssertSucceeded( ) ;
  ...
  if ( geoPointPosition.GetCount() != 0 ) {
    ...
    if ( arrayBlendData.GetCount() == datas->arrayPoints.size() ) {
      for ( ULONG iPnt=0 ; iPnt<datas->arrayPoints.size() ; iPnt++ ) {
        datas->arrayPoints.at(iPnt).position.LinearlyInterpolate ( 
          datas->arrayPoints.at(iPnt).position,
          datas->arrayPoints.at(iPnt).smoothPosition, 
          arrayBlendData[iPnt] 
        );
