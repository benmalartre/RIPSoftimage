...
CStatus RegisterIRBuildElementID( PluginRegistrar& in_reg )
{
	...
	st = nodeDef.AddInputPort(ID_IN_NbJoints,ID_G_100,siICENodeDataLong,siICENodeStructureArray,siICENodeContextSingleton,L"NbJoints",L"NbJoints",0,CValue(),CValue(),ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
	st.AssertSucceeded( ) ;

	st = nodeDef.AddInputPort(ID_IN_NbBones,ID_G_100,siICENodeDataLong,siICENodeStructureArray,siICENodeContextSingleton,L"NbBones",L"NbBones",0,CValue(),CValue(),ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
	st.AssertSucceeded( ) ;

	st = nodeDef.AddInputPort(ID_IN_Divisions, ID_G_100, siICENodeDataLong, siICENodeStructureArray, siICENodeContextSingleton, L"Divisions", L"Divisions", 1, CValue(), CValue(), ID_UNDEF, ID_UNDEF, ID_CTXT_CNS);
	st.AssertSucceeded();

	// Add output ports.
	st = nodeDef.AddOutputPort(ID_OUT_ElementID,siICENodeDataLong,siICENodeStructureArray,siICENodeContextSingleton,L"ID",L"ID",ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
	st.AssertSucceeded( ) ;

	st = nodeDef.AddOutputPort(ID_OUT_SubID,siICENodeDataLong,siICENodeStructureArray,siICENodeContextSingleton,L"SubID",L"SubID",ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
	st.AssertSucceeded( ) ;

	st = nodeDef.AddOutputPort(ID_OUT_BaseID,siICENodeDataLong,siICENodeStructureArray,siICENodeContextSingleton,L"BaseID",L"BaseID",ID_UNDEF,ID_UNDEF,ID_CTXT_CNS);
	st.AssertSucceeded( ) ;

	st = nodeDef.AddOutputPort(ID_OUT_JointID, siICENodeDataLong, siICENodeStructureArray, siICENodeContextSingleton, L"JointID", L"JointID", ID_UNDEF, ID_UNDEF, ID_CTXT_CNS);
	st.AssertSucceeded();

	st = nodeDef.AddOutputPort(ID_OUT_U, siICENodeDataFloat, siICENodeStructureArray, siICENodeContextSingleton, L"U", L"U", ID_UNDEF, ID_UNDEF, ID_CTXT_CNS);
	st.AssertSucceeded();
