...
CStatus RegisterIRDeltaMushInit ( PluginRegistrar& in_reg )
{
  ...
  st = nodeDef.AddInputPort(
    DMI_ID_IN_Geometry, DMI_ID_G_100, siICENodeDataGeometry, siICENodeStructureSingle, siICENodeContextComponent0D,
    L"Geometry", L"Geometry", CValue(), CValue(), CValue(), DMI_ID_UNDEF, DMI_ID_UNDEF, DMI_ID_CTXT_CNS
  );
  st.AssertSucceeded( ) ;

  st = nodeDef.AddInputPort(
    DMI_ID_IN_SmoothIteration, DMI_ID_G_100, siICENodeDataLong, siICENodeStructureSingle, siICENodeContextComponent0D,
    L"SmoothIteration", L"SmoothIteration", 10, 0, 100, DMI_ID_UNDEF, DMI_ID_UNDEF, DMI_ID_CTXT_CNS
  );
  st.AssertSucceeded( ) ;

  st = nodeDef.AddInputPort(
    DMI_ID_IN_PointIndex, DMI_ID_G_100, siICENodeDataLong, siICENodeStructureSingle, siICENodeContextComponent0D,
    L"PointIndex", L"PointIndex", 10, 0, 100, DMI_ID_UNDEF, DMI_ID_UNDEF,DMI_ID_CTXT_CNS
  );
  st.AssertSucceeded( ) ;

  // Add output ports.
  st = nodeDef.AddOutputPort(
    DMI_ID_OUT_OutPort, siICENodeDataMatrix44, siICENodeStructureSingle, siICENodeContextComponent0D,
    L"OutPointTransform", L"OutPointTransform"
  );
  st.AssertSucceeded( ) ;
